 ...
 *   2. Invoke `CreateProductUseCase.execute(input)`.
 *   3. Map typed errors via `error-mapper`.
 *   4. Apply JWT middleware (route is protected).
 */
...
/**
 * The `withJwt` middleware is composed on the inner handler at construct
 * time (see `ApiStack`); this file exposes the unauthenticated variant
 * here as `handler`. The protected variant is built by wrapping
 * `withJwt(handler)` at deploy time in `ApiStack.ts`.
 */
