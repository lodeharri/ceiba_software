...
- [x] Create `packages/infra/src/constructs/migrations.ts`: `CustomResource` Lambda that runs `npx prisma migrate deploy` then `npx tsx prisma/seed.ts` in one invocation; reads `DATABASE_URL` from SSM.
